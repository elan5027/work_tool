...
def main():
    ...
    path = args.directory_path + '/internal_wav/'
    file_list = [f for f in os.listdir(path+'wav') if f.endswith(".wav")]
    start = -3
    end = 2
    # 파일 목록 출력
    number = 0
    for file in file_list:
        direct = path + 'wav/' + file
        ...
        if number != json_number:
            ...
            number = json_number
        #print(json_number)
        save_json(path, json_number, direct, start, end, "AI", number=json_number)
        start+=2
        end+=2
